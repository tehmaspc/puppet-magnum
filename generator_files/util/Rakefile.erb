...
def check_magnum_vagrant_box
  ...
  system 'vagrant box list | grep -q \'magnum-vagrant\''
  unless $?.to_i.equal?(0) then
    puts
    puts ('No Vagrant box named \'magnum-vagrant\' found! ' +
          'Downloading the box; this will take some time...').colorize(:red).on_white.underline
    puts
  end
end
